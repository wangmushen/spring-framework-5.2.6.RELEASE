...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		//2.将属性name的值，通过分隔符"," 或者";"进行切分，并将数据添加到aliases中
		...
		//3.如果属性id的值为空，那就取aliases集合中的第一个value的值，作为bean的名称
		String beanName = id;
		...
		//开始深度解析bean标签，并将解析的成果封装为AbstractBeanDefinition
		...
		if (beanDefinition != null) {
			...
			//5.根据解析到的beanDefinition、beanName和aliasesArray，创建一个BeanDefinitionHolder
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			//3.通过属性class和parent的值，初步创建AbstractBeanDefinition
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);

			//4.解析bean标签的各种其他属性，并封装到AbstractBeanDefinition中
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));

			//5.解析bean标签的各种子标签元素，并将解析结果封装到AbstractBeanDefinition中
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) {
				...
				//将属性key和value的信息封装为BeanMetadataAttribute
				BeanMetadataAttribute attribute = new BeanMetadataAttribute(key, value);
				attribute.setSource(extractSource(metaElement));
				//将BeanMetadataAttribute记录到BeanMetadataAttributeAccessor中
			...
			try {
				...
				if (index < 0) {
					...
					try {
						...
						//解析constructor-args标签下的所有属性值
						Object value = parsePropertyValue(ele, bd, null);
						//将constructor-args标签解析的属性值，封装在ValueHolder中
						...
						if (bd.getConstructorArgumentValues().hasIndexedArgumentValue(index)) {
							...
							//将constructor-args标签解析到的所有信息，都封装到beanDefinition中
	...
	public void parsePropertyElement(Element ele, BeanDefinition bd) {
		...
		try {
			...
			//然后将属性值封装成PropertyValue
			PropertyValue pv = new PropertyValue(propertyName, val);
			parseMetaElements(ele, pv);
			pv.setSource(extractSource(ele));
			//最后将PropertyValue设置到BeanDefinition中
